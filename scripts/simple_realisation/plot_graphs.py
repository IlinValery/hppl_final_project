...
if __name__ == '__main__':
    ...
    sorted_data = sorted(zip(text_sizes, advanced_encryptor_times, advanced_decryptor_times), key=lambda x: x[0])
    text_sizes, advanced_encryptor_times, advanced_decryptor_times = zip(*sorted_data)
    text_sizes = list(text_sizes)
    advanced_encryptor_times = list(advanced_encryptor_times)
    advanced_decryptor_times = list(advanced_decryptor_times)
    print(text_sizes)
    fig, ax = plt.subplots()
    ...
    ax.set_title("Advanced crypting results")
    plt.grid(zorder=0)
    fig.tight_layout()
